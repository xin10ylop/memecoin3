#!/usr/bin/env python3
"""Analyse the RETRO-HARVESTED sample — the honest young-pool measurement.
...
def load(db_path: str, min_age_hours: float):
    ...
    cohort = pd.read_sql_query(
        """SELECT r.pool_address, p.base_symbol AS symbol, p.dex_id,
                  p.pool_created_at, r.n_bars
           FROM retro_harvest r JOIN pools p
             ON p.pool_address = r.pool_address
           WHERE r.n_bars > 0""", db)
    if not cohort.empty:
        cts = pd.to_datetime(cohort.pool_created_at, errors="coerce", utc=True)
        cohort["created_ts"] = (
            cts - pd.Timestamp("1970-01-01", tz="UTC")).dt.total_seconds()
        cohort = cohort.dropna(subset=["created_ts"])
    if cohort.empty:
        return cohort, {}
    bars = {}
...
def main() -> int:
    ...
    print(f"unbiased sample: {len(cohort)} pools harvested, bars for {len(bars)}")
    if cohort.empty:
